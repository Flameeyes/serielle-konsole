...
K_EXPORT_COMPONENT_FACTORY( kcm_serielle, ModuleFactory("kcmserielle") )

KCMKonsole::KCMKonsole(QWidget * parent, const char *name, const QStringList&)
:KCModule(ModuleFactory::instance(), parent, name)
{
    
    setQuickHelp( i18n("<h1>Serielle Konsole</h1> With this module you can configure Serielle "
		       "Konsole, the KDE serial terminal application. You can configure the "
		       "generic Serielle Konsole options (which can also be configured using "
		       "the RMB) and you can edit the schemas and sessions available to Serielle "
		       "Konsole."));
    ...
    KAboutData *ab=new KAboutData( "kcmserielle", I18N_NOOP("KCM Serielle Konsole"),
       "0.2",I18N_NOOP("KControl module for Serielle Konsole configuration"), KAboutData::License_GPL,
       "(c) 2007, Diego Pettenò", 0, 0, "flameeyes@gmail.com");

    ab->addAuthor("Diego Pettenò",0, "flameeyes@gmail.com");
...
void KCMKonsole::save()
{
    ...
    dcc->send("seriellekonsole-*", "seriellekonsole", "reparseConfiguration()", QByteArray());
